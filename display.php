...
$flag=0;
$date=date("Y-m-d");
$result=mysqli_query($connect,"select * from attendance_records where date='$date';");
...
if(isset($_POST['submit']) and !empty($_POST['name']) and !empty($_POST['roll_no']) and !empty($_POST['status'])){
        ...
        if(mysqli_num_rows($result)==0){
            foreach($_POST['status'] as $id=>$status){
                ...
                $result=mysqli_query($connect,"insert into attendance_records(name,roll_no,status,date)
                 values('$student_name','$roll_number','$status','$date');");
                 if($result){
                    $flag=1;
                 }
                }
